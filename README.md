...
The `excluded_dirs` option controls which directory names are silently skipped during scanning
...
| Directory | Purpose |
|-----------|---------|
| `@eaDir` | Thumbnails and metadata generated by Synology Photos / Photo Station |
| `@SynoEAStream` | Extended attribute streams (DSM filesystem feature) |
| `@Recycle` | Shared folder recycle bin |
...
| `@Recently-Snapshot` | Btrfs snapshot staging area |
